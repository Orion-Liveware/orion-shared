...
pub enum SecretError {
    ...
    Entry { key: String, source: keyring::Error },

    #[error("Failed to store secret for key '{key}': {source}")]
    Set { key: String, source: keyring::Error },

    #[error("Failed to read secret for key '{key}': {source}")]
    Get { key: String, source: keyring::Error },

    #[error("Failed to delete secret for key '{key}': {source}")]
    Delete { key: String, source: keyring::Error },
}
...
pub fn set_secret(service: &str, key: &str, value: &str) -> Result<(), SecretError> {
    let entry = keyring::Entry::new(service, key).map_err(|e| SecretError::Entry {
        key: key.to_string(),
        source: e,
    })?;
    let trimmed = value.trim();
    entry.set_password(trimmed).map_err(|e| SecretError::Set {
        key: key.to_string(),
        source: e,
    })?;
